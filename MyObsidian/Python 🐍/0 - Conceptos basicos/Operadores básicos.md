...
#Ahora formatearemos el resultado de esta exponensiacion metiendo una coma cada 3 numero y cambiandola a un .
...
```python
odd_numbers = [1, 2, 4, 5, 6, 7]
even_numbers = [4, 6, 8, 2, 9, 3]

#crea dos tuplas con las dos listas
result = zip(odd_numbers, even_numbers)
for element in result:
	print(element)
  
#sumamos cada elemento de las dos tuplas y lo paso a lista
#aca map toma sum como funcion y luego itera sobre el zip
result = list(map(sum, zip(odd_numbers, even_numbers)))
print(result)

#map tiene dos argumentos (una funcion y luego un iterable)
```